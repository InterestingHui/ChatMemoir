...
from memoir.db_v4.audio2text import Audio2TextDB
from memoir.db_v4.biz_message import BizMessageDB
from memoir.db_v4.emotion import EmotionDB
from memoir.db_v4.media import MediaDB
from memoir.db_v4 import ContactDB, HeadImageDB, SessionDB, MessageDB, HardLinkDB
from memoir.db_main import ArchiveInterface, Context
from memoir.model.contact import Contact, ContactType, Person
from memoir.model import Me
from memoir.parser.util.protocbuf.roomdata_pb2 import ChatRoomData
from memoir.parser.parser_v4 import FACTORY_REGISTRY, Singleton
from memoir.log import logger
from memoir.parser.util.protocbuf import contact_pb2
...
class ArchiveV4(ArchiveInterface):
    def __init__(self):
        super().__init__()
        self.db_dir = ''
    ...
    def create_contact(self, contact_info_list) -> Person:
        uid, local_type, flag = contact_info_list[0], contact_info_list[2], contact_info_list[3]
        ...
        label_list = []
        region = ('', '', '')
        if not (uid.endswith('@openim') or uid.endswith('@chatroom')):
            try:
                # 创建顶级消息对象
    ...
    def get_contact_by_username(self, uid: str) -> Person:
        contact_info_list = self.contact_db.get_contact_by_username(uid)
        if contact_info_list:
            contact = self.create_contact(contact_info_list)
            return contact
        else:
    ...
    def _get_chatroom_name(self, uid):
        chatroom = self.contact_db.get_chatroom_info(uid)
