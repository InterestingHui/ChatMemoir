...
class Person:
    ...
    def is_chatroom(self):
        return self.uid.endswith('@chatroom')  # 是否是群聊

    def is_public(self):
        return self.uid.startswith('gh')  # 是否是公众号

    def is_open_im(self):
        return self.uid.endswith('@openim')  # 是否是企业微信联系人
...
class Contact(Person):
    ...
    #     super().__init__()
    #     self.uid: str = contact_info.get('UserName')
    #     self.is_chatroom = self.uid.__contains__('@chatroom')  # 是否是群聊
    #     self.is_open_im = self.uid.endswith('@openim')  # 是否是企业微信联系人
    #     self.is_public = self.uid.startswith('gh')
    #     self.is_unknown = False  # 是否是联系人表中没有的数据
    #     if self.uid.endswith('@stranger'):
    #         self.uid = self.uid[-16:]
    ...
    def load_from_json(self, json_file):
        if os.path.exists(json_file):
            with open(json_file, 'r', encoding='utf-8') as f:
                ...
                self.uid = dic.get('username', '')
                self.data_dir = dic.get('data_dir', '')
