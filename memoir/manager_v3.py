...
from memoir.db_main import ArchiveInterface
from memoir.db_v3.audio2text import Audio2TextDB
from memoir.db_v3.hard_link_file import HardLinkFile
from memoir.db_v3.hard_link_image import HardLinkImage
from memoir.db_v3.hard_link_video import HardLinkVideo

from memoir.db_v3.misc import Misc
from memoir.db_v3.msg import Msg
from memoir.db_v3.media_msg import MediaMsg
from memoir.db_v3.emotion import Emotion
from memoir.db_v3.open_im_contact import OpenIMContactDB
from memoir.db_v3.open_im_media import OpenIMMediaDB
from memoir.db_v3.open_im_msg import OpenIMMsgDB
from memoir.db_v3.public_msg import PublicMsg
from memoir.db_v3.micro_msg import MicroMsg
from memoir.db_v3.favorite import Favorite
from memoir.log import logger
from memoir.model.contact import Contact, Me, ContactType, Person
from memoir.parser.file_parser import get_image_type
from memoir.parser.util.protocbuf.roomdata_pb2 import ChatRoomData
from memoir.parser.parser_v3 import FACTORY_REGISTRY, parser_sub_type, Singleton
...
type_name_dict = {
    ...
    (49, 51): MessageType.FavoritesVideo,
...
def parser_messages(messages, username, db_dir=''):
    ...
    if username.endswith('@chatroom'):
        ...
        contacts = {
            Me().uid: context.get_contact_by_username(Me().uid),
...
class ArchiveV3(ArchiveInterface):
    ...
    def update_audio_to_text(self):
        ...
        contacts_set = {contact.uid for contact in contacts}
        for message in messages:
            if message[2] == 34:
            ...
            uid = message[11]
            # if uid not in contacts_set:
            #     contact = ContactDefault(uid)
            #     self.micro_msg_db.add_contact(contact)
            #     contacts_set.add(uid)
    ...
    def create_contact(self, contact_info_list) -> Person:
        ...
        uid = contact_info_list[0]
        nickname = contact_info_list[4]
        remark = contact_info_list[3]
        if not nickname and uid.endswith('@chatroom'):
    ...
    def get_contact_by_username(self, uid: str) -> Contact:
        if uid.endswith('@openim'):
            contact_info_list = self.open_contact_db.get_contact_by_username(uid)
            ...
            contact_info_list = self.micro_msg_db.get_contact_by_username(uid)
            if contact_info_list:
                contact = self.create_contact(contact_info_list)
            else:
                contact = Contact(
    ...
    def get_chatroom_members(self, chatroom_name) -> dict[Any, Contact] | Any:
        ...
        for mem in parsechatroom.members:
            ...
            if contact:
                ...
                result[contact.uid] = contact
        self.chatroom_members_map[chatroom_name] = result
        return result

    def _get_chatroom_name(self, uid):
        """
        获取没有命名的群聊名
        :param uid:
        :return:
        """
        chatroom = self.micro_msg_db.get_chatroom_info(uid)
