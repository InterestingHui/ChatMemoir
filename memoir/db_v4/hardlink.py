...
from memoir.merge import increase_data
from memoir.model.db_model import ArchiveBase
from memoir.log import logger
from memoir.model.message import Message
from memoir.parser.util.protocbuf import file_info_pb2
...
class HardLinkDB(ArchiveBase):
    def get_image_path(self):
        pass

    def create_index(self):
        sql = "CREATE INDEX IF NOT EXISTS image_hardlink_info_v4_md5 ON image_hardlink_info_v4(md5);"
        try:
    ...
    def get_image_by_time(self, message: Message, talker_username):
        ...
        if os.path.exists(os.path.join(Me().data_dir, path1)):
            return path1
        else:
    ...
    def get_image(self, content, message, up_dir="", md5=None, thumb=False, talker_username='') -> str:
        ...
        if thumb:
            ...
            if os.path.exists(os.path.join(Me().data_dir, result)):
                return result
        if not md5:
    ...
    def merge(self, db_path):
        ...
        try:
            # 获取列名
            increase_data(db_path, self.cursor, self.DB, 'file_hardlink_info_v4', 'md5', exclude_column='_rowid_')
