...
from memoir.model.message import VoipMessage, BusinessCardMessage, MergedMessage, FavoritesVideoMessage, \
    PositionMessage, TransferMessage, RedEnvelopeMessage, FavNoteMessage, PatMessage
from memoir.parser.link_parser import parser_link, parser_voip, parser_applet, parser_business, \
...
from memoir.parser.util.protocbuf import packed_info_data_pb2, packed_info_data_merged_pb2, packed_info_data_img_pb2, \
...
class Singleton:
    ...
    def common_attribute(self, message, username, manager):
        ...
        return is_sender, uid, message_content
...
class VideoMessageFactory(MessageFactory, Singleton):
    def create(self, message, username, manager):
        ...
        if filename:
            ...
            if os.path.exists(os.path.join(Me().data_dir, video_path)):
                msg.path = video_path
...
class MergedMessageFactory(MessageFactory, Singleton):
    def create(self, message, username, manager):
        ...
        rec_dir = os.path.join(Me().data_dir, 'msg', 'attach', hashlib.md5(username.encode("utf-8")).hexdigest(), month,
                               'Rec')
