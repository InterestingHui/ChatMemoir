...
from memoir.model.message import BusinessCardMessage, VoipMessage, MergedMessage, FavoritesVideoMessage, \
    PositionMessage, TransferMessage, RedEnvelopeMessage, FavNoteMessage, PatMessage
from memoir.parser.link_parser import parser_link, parser_applet, parser_business, parser_voip, \
...
from memoir.parser.util.protocbuf.msg_pb2 import MessageBytesExtra
from memoir.parser.parser_v4 import LimitedDict
...
from memoir.model import Message, TextMessage, ImageMessage, VideoMessage, EmojiMessage, LinkMessage, FileMessage, \
    AudioMessage, QuoteMessage, MessageType
from memoir.model import Me
...
class MessageFactory(ABC):
    @abstractmethod
    def create(self, data, username: str, database_manager: ArchiveInterface):
        """
        创建一个Message实例
...
class Singleton:
    ...
    @classmethod
    def get_contact(cls, uid, database_manager: ArchiveInterface):
        if uid in cls.contacts:
            return cls.contacts[uid]
        else:
            contact = database_manager.get_contact_by_username(uid)
            cls.contacts[uid] = contact
            return contact
    ...
    def common_attribute(self, message, username, manager):
        ...
        :param username:
        :param manager:
        :return: uid,is_sender,xml_content
        """
        is_sender = message[4]
        uid = ''
        if is_sender:
            ...
            if username.endswith('@chatroom'):
                ...
                if ':' in uid:  # wxid_ewi8gfgpp0eu22:25319:1
                    uid = uid.split(':')[0]
            else:
                uid = username
        if uid not in self.contacts:
            self.contacts[uid] = manager.get_contact_by_username(uid)
        ...
        return is_sender, uid, xml_content if xml_content else message[7]
...
class UnknownMessageFactory(MessageFactory, Singleton):
    def create(self, message, username, manager):
        is_sender, uid, xml_content = self.common_attribute(message, username, manager)
        return Message(
            local_id=message[0],
            server_id=message[9],
            sort_seq=message[5],
            ...
            is_sender=is_sender,
            sender_id=uid,
            display_name=self.contacts[uid].remark,
            avatar_src=self.contacts[uid].small_head_img_url,
            status=message[6],
...
class LinkMessageFactory(MessageFactory, Singleton):
    def create(self, message, username, manager):
        is_sender, uid, message_content = self.common_attribute(message, username, manager)
        msg = LinkMessage(
            local_id=message[0],
            server_id=message[9],
            sort_seq=message[5],
            ...
            for index, inner_msg in enumerate(merged_messages):
                if inner_msg.type == MessageType.Image:
                    if dir0:
                        ...
                        origin_img_path = os.path.join(Me().data_dir,
                                                       img_suffix)
                    else:
                        ...
                        inner_msg.thumb_path = manager.get_image(content='', md5=inner_msg.md5, bytesExtra=b'',
                                                                 up_dir='',
                                                                 thumb=True, talker_username=username)
                    if not os.path.exists(os.path.join(Me().data_dir, inner_msg.path)) or inner_msg.path == '.':
...
class FavoritesVideoMessageFactory(MessageFactory, Singleton):
    def create(self, message, username, manager):
        is_sender, uid, message_content = self.common_attribute(message, username, manager)
        msg = FavoritesVideoMessage(
            local_id=message[0],
            server_id=message[9],
            sort_seq=message[5],
            timestamp=message[5],
            str_time=message[8],
            type=MessageType.FavoritesVideo,
            talker_id=username,
            is_sender=is_sender,
            sender_id=uid,
            display_name=self.contacts[uid].remark,
...
FACTORY_REGISTRY = {
    ...
    MessageType.FavoritesVideo: FavoritesVideoMessageFactory(),
