...
# 读取微信信息(account,mobile,name,mail,uid,key)
def read_info(version_list):
    ...
    for process in psutil.process_iter(['name', 'exe', 'pid']):
        if process.name() == 'WeChat.exe':
            ...
            tmp_rd['uid'] = get_info_wxid(Handle)
            tmp_rd['data_dir'] = get_wx_dir(tmp_rd['uid']) if tmp_rd['uid'] != "None" else "None"
            tmp_rd['key'] = "None"
            tmp_rd['key'] = get_key(tmp_rd['data_dir'], addrLen)
            if tmp_rd['key'] == 'None':
                tmp_rd['errcode'] = 404
...
def get_info_v4():
    ...
    for process in psutil.process_iter(['name', 'exe', 'pid']):
        if process.name() == 'Weixin.exe':
            ...
            result_v4.append(
                {
                    'uid': session_legacy.uid,
                    'name': session_legacy.nick_name,
                    'account': session_legacy.account_name,
                    'key': session_legacy.key,
                    'mobile': session_legacy.phone,
                    'version': session_legacy.version,
                    'data_dir': session_legacy.data_dir,
                    'errcode': 200
                }
            )
    return result_v4
...
if __name__ == "__main__":
    ...
    session_info = get_info_v3(version_list)
    print(session_info)
