...
@File        : MemoTrace-session_info_v3.py 
@Description : 
"""

# -*- coding: utf-8 -*-#
# -------------------------------------------------------------------------------
# Name:         getsession_legacy.py
# Description:
# Author:       xaoyaoo
# Date:         2023/08/21
# -------------------------------------------------------------------------------
...
def get_info_wxid(h_process):
    ...
    uid = max(wxids, key=wxids.count) if wxids else "None"
    return uid
...
def get_wx_dir(uid):
    ...
    try:
        ...
        msg_dir = os.path.join(w_dir, "WeChat Files", uid)
        return msg_dir
...
def dump_session_info_v3(version_list, pid) -> SessionInfo:
    ...
    if wechat_base_address == 0:
        session_info.errmsg = '错误！请登录微信。'
        return session_info

    Handle = ctypes.windll.kernel32.OpenProcess(0x1F0FFF, False, process.pid)

    bias_list = version_list.get(session_info.version)
    if not isinstance(bias_list, list) or len(bias_list) <= 4:
        session_info.errcode = 405
        session_info.errmsg = '错误！微信版本不匹配，请手动填写信息。'
        return session_info
    else:
        ...
        session_info.account_name = get_info_without_key(Handle, account__base_address, 32) if bias_list[1] != 0 else "None"
        session_info.phone = get_info_without_key(Handle, mobile_base_address, 64) if bias_list[2] != 0 else "None"
        session_info.nick_name = get_info_without_key(Handle, name_base_address, 64) if bias_list[0] != 0 else "None"

    addrLen = get_exe_bit(process.exe()) // 8

    session_info.uid = get_info_wxid(Handle)
    session_info.data_dir = get_wx_dir(session_info.uid)
    session_info.key = get_key(session_info.data_dir, addrLen)
    if not session_info.key:
        session_info.errcode = 404
        session_info.errmsg = '请重启微信后重试。'
    else:
