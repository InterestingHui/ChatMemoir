...
修复 ChatMemoir（留痕）项目的微信数据库解密功能，使其兼容微信 4.x 系列（当前测试版本 4.1.8.29）。ChatMemoir 是一个微信聊天记录提取和导出工具，采用三步流水线：解密数据库 → 查看联系人 → 导出聊天记录。当前解密步骤因微信版本更新导致内存特征匹配失败，需要更新密钥提取逻辑。
...
## Languages
- Python 3.10+ (header comment in `memoir/__init__.py` specifies `Python3.10`; type union syntax `str | bytes` used throughout, requiring 3.10+)
- Used for all core modules: `memoir/`, `scribe/`, `mind/`, and entry-point scripts
- Protocol Buffers (`.proto` files in `memoir/parser/util/protocbuf/`) - compiled to Python (`*_pb2.py`)
- YAML (GitHub Actions workflow in `.github/workflows/sync.yml`)
- HTML/CSS/JS (export template in `scribe/resources/template.html`)
...
- Secondary requirements: `mind/qwen2-0.5b/requirements.txt` (AI sub-project)
...
- Abstract base class pattern for database interface (`memoir/db_main.py`)
- FastAPI + Uvicorn - OpenAI-compatible API server (`mind/api_server.py`)
- Gradio - UI for Qwen model inference (`mind/qwen2-0.5b/app.py`)
- PyTorch + Transformers (HuggingFace) - Model loading and inference
- DashInfer - Alternative inference engine for Qwen 2.0
## Key Dependencies
- `sqlite3` (stdlib) - All database access uses Python's built-in sqlite3 module directly. No ORM. Raw SQL queries via cursors. Files: `memoir/model/db_model.py`, `memoir/merge.py`, all `memoir/db_v3/*.py` and `memoir/db_v4/*.py`
- `pycryptodome` (Crypto) - AES-CBC decryption of WeChat database files. Files: `memoir/decrypt/decrypt_v3.py`, `memoir/decrypt/decrypt_v4.py`, `memoir/decrypt/decrypt_dat.py`
...
- `pymem==1.14.0` - Read WeChat process memory to extract encryption keys. Files: `memoir/decrypt/session_v3.py`, `memoir/decrypt/session_v4.py`
- `psutil~=6.1.1` - Process enumeration (find WeChat PID, memory maps). Files: `memoir/decrypt/__init__.py`, `memoir/decrypt/common.py`
- `pywin32==308` - Windows API access (`win32api.GetFileVersionInfo`, `win32com.client.Dispatch`, `winreg`). Files: `memoir/decrypt/common.py`, `memoir/decrypt/get_bias_addr.py`
- `yara-python` - Pattern matching for WeChat version detection / memory scanning. File: `memoir/decrypt/session_v4.py`
- `protobuf==4.25.1` + `google==3.0.0` - Deserialize protobuf-encoded WeChat data (contacts, messages, room data, file info, emoji descriptions). Files: `memoir/parser/util/protocbuf/*_pb2.py`, `memoir/parser/emoji_parser.py`, `memoir/parser/parser_v3.py`
- `xmltodict~=0.14.2` - Parse XML content in WeChat messages (audio, files, links, emojis). Files: `memoir/parser/audio_parser.py`, `memoir/parser/file_parser.py`, `memoir/parser/parser_v3.py`, `memoir/model/message.py`
- `lz4~=4.3.3` - Decompress LZ4-compressed message data. File: `memoir/parser/parser_v3.py`
- `zstandard~=0.23.0` - Decompress Zstandard-compressed data (v4 format). File: `memoir/manager_v4.py`
- `docx` (python-docx) - Word document export. File: `scribe/exporter_docx.py`
- `openpyxl==3.1.5` - Excel spreadsheet export. File: `scribe/exporter_xlsx.py`
- `pillow==11.0.0` (PIL) - Image processing, format detection, thumbnail generation. Files: `scribe/exporter_xlsx.py`, `memoir/db_v4/head_image.py`
- `pysilk-mod==1.6.4` - SILK audio codec decoding for WeChat voice messages. File: `scribe/exporter.py`
- `requests~=2.32.3` - HTTP requests (emoji downloads, etc.)
- `aiofiles~=24.1.0` - Async file I/O for batch image decryption. File: `memoir/decrypt/decrypt_dat.py`
...
- `ffmpeg.exe` (bundled binary in `scribe/ffmpeg.exe` and `scribe/resources/ffmpeg.exe`) - Audio/video conversion via subprocess
## Database Architecture
# From memoir/model/db_model.py
...
|-----------|---------|-----------|
| `memoir/` | Core library: decrypt, parse, query WeChat databases | `__init__.py`, `db_main.py`, `manager_v3.py`, `manager_v4.py` |
| `memoir/decrypt/` | Extract encryption keys from WeChat process, decrypt DBs | `session_v3.py`, `session_v4.py`, `decrypt_v3.py`, `decrypt_v4.py`, `decrypt_dat.py` |
| `memoir/db_v3/` | Database access layer for WeChat 3.x schema | `msg.py`, `micro_msg.py`, `media_msg.py`, etc. |
| `memoir/db_v4/` | Database access layer for WeChat 4.0 schema | `message.py`, `contact.py`, `media.py`, etc. |
...
- Strategy pattern for WeChat v3 vs v4 support via `ArchiveInterface` abstract class
- Factory pattern for message type parsing (`MessageFactory` subclasses registered in `FACTORY_REGISTRY`)
- Direct SQLite access (no ORM) through a base class `ArchiveBase` that handles multi-database series (e.g., `message_0.db`, `message_1.db`)
...
- Location: `memoir/db_v3/`, `memoir/db_v4/`, `memoir/db_main.py`, `memoir/model/db_model.py`
- Contains: Per-table database classes inheriting `ArchiveBase`
- Depends on: `sqlite3` (stdlib), protobuf definitions in `memoir/parser/util/protocbuf/`
...
- Location: `memoir/manager_v3.py`, `memoir/manager_v4.py`, `memoir/__init__.py`
- Contains: `ArchiveV3`, `ArchiveV4` implementing `ArchiveInterface`
...
- Depends on: Manager layer (`ArchiveInterface`), file I/O utilities
- Used by: Entry point scripts `3-exporter.py`
### Layer 6: Model
- Purpose: Define data structures shared across all layers
- Location: `memoir/model/`
- Contains: `Message` dataclass hierarchy, `Contact`/`Person`/`Me` dataclasses, `ArchiveBase` SQLite wrapper, `MessageType` constants
...
### Export Flow (Step 3)
### State Management
- **`Me` singleton** (`memoir/model/contact.py`): Stores current user's uid, name, data_dir, xor_key. Loaded from `info.json` at init.
- **`Singleton` class** (`memoir/parser/parser_v4.py`): Shared across all `MessageFactory` instances in a process. Holds a contact cache and a `LimitedDict` message cache (100 entries) for quote resolution.
- **`contacts_map` / `chatroom_members_map`**: In-memory caches on each `ArchiveInterface` instance for contact lookups.
## Key Abstractions
### `ArchiveInterface` (Abstract Base Class)
- Purpose: Defines the full API for querying WeChat data (contacts, messages, media, etc.)
- Examples: `memoir/db_main.py`
- Pattern: Abstract base class with ~40 methods. Subclassed by `ArchiveV3` and `ArchiveV4`.
- Consumers create via `ArchiveConnection(db_dir, version).get_interface()` in `memoir/__init__.py`.
### `ArchiveBase` (SQLite Wrapper)
- Purpose: Handles opening/closing SQLite databases, supports series (multi-file) databases
- Examples: `memoir/model/db_model.py`
...
- Examples: `memoir/parser/parser_v4.py` (17 factory classes), `memoir/parser/parser_v3.py`
...
| Aspect | V3 | V4 |
|--------|----|----|
| Manager | `memoir/manager_v3.py` `ArchiveV3` | `memoir/manager_v4.py` `ArchiveV4` |
| DB modules | `memoir/db_v3/` (13 files) | `memoir/db_v4/` (10 files) |
| Message tables | `MSG0.db` with table `MSG_{localId}` | `message_0.db` with table `Msg_{md5(uid)}` |
