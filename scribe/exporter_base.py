...
from memoir import MessageType, ArchiveInterface
from memoir.model import Contact, Me, Message
...
def safe_makedirs(path):
    """安全创建目录。已存在则跳过，避免 Windows 上 os.makedirs(exist_ok=True) 的 PermissionError。"""
...
def makedirs(path):
    """创建导出目录结构。"""
    safe_makedirs(path)
    for d in ('image', 'emoji', 'video', 'voice', 'file', 'avatar', 'music', 'icon'):
        safe_makedirs(os.path.join(path, d))
...
class ExporterBaseBase:
    ...
    def __init__(self):
        ExporterBaseBase.scribe_id += 1
        self.id = ExporterBaseBase.scribe_id
...
class ExporterBase(ExporterBaseBase):
    ...
    def __init__(
        ...
        self.origin_path = os.path.join(output_dir, '聊天记录', f'{self.contact.remark}({self.contact.uid})')
    ...
    def _is_select_by_contact(self, message):
        ...
        if self.contact.is_chatroom() and self.group_members_set:
            ...
            if uid in self.group_members_set:
                return True
            else:
                return False
        else:
    ...
    def save_avatars(self):
        if self.contact.is_chatroom():
            self.group_contacts = self.database.get_chatroom_members(self.contact.uid)
            self.group_contacts[Me().uid] = Me()
        else:
            self.group_contacts = {
                Me().uid: Me(),
                self.contact.uid: self.contact
            }
        for uid, contact in self.group_contacts.items():
            self.save_avatar(contact)

    def save_avatar(self, contact):
        avatar_buffer = self.database.get_avatar_buffer(contact.uid)
        avatar_path = os.path.join(self.origin_path, 'avatar', f'{contact.uid}.png')
    ...
    def get_avatar_path(self, message: Message, is_absolute_path=False) -> str | int:
        ...
        if is_absolute_path:
            ...
            if message.sender_id in self.group_contacts:
                ...
                self.group_contacts[contact.uid] = contact
        else:
            if self.contact.is_chatroom():
    ...
    def get_avatar_urls(self):
        index = 0
        if self.contact.is_chatroom():
            messages = msg_db.get_messages(self.contact.uid, time_range=self.time_range)
            for message in messages:
                contact = message[13]
                if contact.uid not in self.avatar_urls_dict:
                    avatar_path = os.path.join(self.origin_path, 'avatar', f'{contact.uid}.png')
                    contact.save_avatar(avatar_path)
                    self.avatar_urls.append(contact.small_head_img_url)
                    self.avatar_paths.append(f'./avatar/{contact.uid}.png')
                    self.avatar_urls_dict[contact.uid] = index
                    index += 1
        else:
            self.avatar_urls = [Me().small_head_img_url, self.contact.small_head_img_url]
            avatar_path = os.path.join(self.origin_path, 'avatar', f'{Me().uid}.png')
            QMe().save_avatar(avatar_path)
            avatar_path1 = os.path.join(self.origin_path, 'avatar', f'{self.contact.uid}.png')
            # self.contact.save_avatar(avatar_path1)
            self.avatar_paths = [f'./avatar/{Me().uid}.png', f'./avatar/{self.contact.uid}.png']
    ...
    def get_avatar_paths(self):
        ...
        if self.contact.is_chatroom():
            ...
            for message in messages:
                contact = message[13]
                if contact.uid not in self.avatar_paths_dict:
                    self.avatar_paths.append(contact.small_head_img_url)
                    self.avatar_paths_dict[contact.uid] = index
                    index += 1
        else:
...
class ImageExporter(ExporterBaseBase):
    ...
    def run(self):
        ...
        num = len(messages)
        safe_makedirs(os.path.join(config.OUTPUT_DIR, 'image'))
        for index, message in enumerate(messages):
            ...
            if type_ == 3:
                ...
                image_path = hard_link_db.get_image(str_content, BytesExtra, up_dir=Me().data_dir, thumb=False)
...
class VideoExporter(ExporterBaseBase):
    def run(self):
        ...
        messages = database.get_messages_all()
        num = len(messages)
        safe_makedirs(os.path.join(config.OUTPUT_DIR, 'video'))
        for index, message in enumerate(messages):
            type_ = message[2]
            timestamp = message[5]
            # 把时间戳转换为格式化时间
            ...
            if type_ == 43:
                ...
                if video_path:
                    video_path = f'{Me().data_dir}/{video_path}'
...
class FileExporter(ExporterBaseBase):
    def run(self):
        ...
        safe_makedirs(os.path.join(config.OUTPUT_DIR, 'files'))
        for index, message in enumerate(messages):
            type_ = message[2]
            sub_type = message[3]
            timestamp = message[5]
        ...
        try:
            ...
            with open(self.output_path, mode='w', newline='', encoding='utf-8-sig') as file:
                ...
                for contact in contacts:
                    writer.writerow(
                        [contact.uid, contact.alias, contact.flag, contact.remark, contact.nickname,
...
class GroupContactExporter(ExporterBaseBase):
        ...
        if self.contact:
            if not isinstance(self.contact, list):
                self.origin_path = os.path.join(output_dir, '聊天记录',
                                                f'{self.contact.remark}({self.contact.uid})')
                safe_makedirs(self.origin_path)
    ...
    def run(self):
        ...
        contacts = self.database.get_chatroom_members(self.contact.uid)
        try:
            # 写入CSV文件
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        for source_file, output_dir, dst_name in file_tasks:
            ...
            if not os.path.exists(os.path.dirname(destination_file)):
                safe_makedirs(os.path.dirname(destination_file))
