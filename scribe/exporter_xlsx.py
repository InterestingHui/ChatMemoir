...
from memoir import Me, MessageType
...
from scribe.exporter_base import ExporterBase, copy_files, decode_audios, get_new_filename, safe_makedirs
...
from memoir.parser.link_parser import wx_sport, wx_collection_data, wx_pay_data
...
class ExcelExporter(ExporterBase):
    ...
    def add_member_info(self, sheet):
        if self.contact.is_chatroom():
            columns = ['uid', '微信号', '类型', '群昵称', '昵称', '头像地址',
            ...
            if self.contact.is_public():
                ...
                columns = (
                    'uid', '微信号', '类型', '群昵称', '昵称', '头像地址', '头像原图', '标签', '性别', '电话',
                    '个性签名', '国家（地区）', '省份', '城市')
                # 写入CSV文件
                sheet.append(columns)
    ...
    def to_excel(self):
        ...
        def parser_merged(merged_message):
            for msg in merged_message.messages:
                ...
                if type_ == MessageType.Image:
                    ...
                    image_tasks.append(
                        (
                            os.path.join(Me().data_dir, msg.path),
                    ...
                    image_tasks.append(
                        (
                            os.path.join(Me().data_dir, msg.thumb_path),
                            os.path.join(image_dir, msg.str_time[:7]),
                            msg.file_name + '_t'
                        )
                    )
                    ...
                    origin_file_path = os.path.join(Me().data_dir, msg.path)
                    file_tasks.append(
                        (
                            origin_file_path,
        ...
        for index, message in enumerate(messages):
            ...
            if type_ == MessageType.Image:
                ...
                image_tasks.append(
                    (
                        os.path.join(Me().data_dir, message.path),
                ...
                image_tasks.append(
                    (
                        os.path.join(Me().data_dir, message.thumb_path),
                        os.path.join(image_dir, message.str_time[:7]),
                ...
                origin_file_path = os.path.join(Me().data_dir, message.path)
                file_tasks.append(
                    (
                        origin_file_path,
    ...
    def run(self):
        if self.contact.is_public():
            if self.contact.uid == 'gh_3dfda90e39d6':
                self.wx_pay()
            elif self.contact.uid == 'gh_f0a92aa7146c':
                self.wx_collect()
            elif self.contact.uid == 'gh_43f2581f6fd6':
                self.wx_sport()
            else:
